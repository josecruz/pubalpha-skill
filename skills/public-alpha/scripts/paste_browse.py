...
Query commands (so the agent/CLI has the same info as the web — read results/paste.json, build if missing):
    ...
    python3 skills/public-alpha/scripts/paste_browse.py --shows         # list shows
    python3 skills/public-alpha/scripts/paste_browse.py --list [--show threadguy]   # list episodes
    python3 skills/public-alpha/scripts/paste_browse.py --stream <id>   # an episode's calls
    python3 skills/public-alpha/scripts/paste_browse.py --speakers      # top speakers
    python3 skills/public-alpha/scripts/paste_browse.py --speaker <handle>          # a speaker's calls
Content is from paste.trade (allowed public surface) and credited.
"""
import argparse
import json
import sys
...
def build() -> dict:
    """Pull the allowed surface, enrich with collision-guarded since-call %, write paste.json."""
    data = PasteTradeSource().browse()
    eps = data["episodes"]
    print(f"  {len(data['shows'])} shows · {len(eps)} episodes · {len(data['speakers'])} speakers")
    market = _cmc()
    if market is not None and eps:
        try:
            mkt = market.market_block(sorted({t["ticker"] for e in eps for t in e["trades"]}))
        except Exception as e:
        ...
        for e in eps:
            for t in e["trades"]:
                ...
                n += 1
        print(f"  since-call % attached for {n} trades (CMC-priceable, collision-guarded)")
...
def main():
    ...
    ap.add_argument("--show", metavar="SLUG", help="filter --list to a show (threadguy|all-in)")
    ap.add_argument("--stream", metavar="ID", help="show an episode's calls")
    ap.add_argument("--speakers", action="store_true", help="list top speakers")
    ap.add_argument("--speaker", metavar="HANDLE", help="a speaker's calls")
    a = ap.parse_args()

    if not any([a.shows, a.list, a.stream, a.speakers, a.speaker]):
        build()
        return
    ...
    if a.shows:
        for s in d["shows"]:
            print(f"{s['slug']:10} {s['platform']:8} @{s['streamer']:14} {s['n_episodes']} eps · {s['n_trades']} calls")
    if a.list:
        for e in d["episodes"]:
    ...
    if a.speaker:
        ...
        print(f"@{s['handle']} {'(verified)' if s['verified'] else ''} · {s['n_calls']} calls · {s['long']}L/{s['short']}S · {s['n_episodes']} eps · {','.join(s['shows'])}")
        for e in d["episodes"]:
            for t in e["trades"]:
